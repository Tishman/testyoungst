...
extension Effect {
  ...
  public func cancellable(id: AnyHashable, cancelInFlight: Bool = false, bag: CancellationBag) -> Effect {
  ...
  ///   identifier.
  public static func cancel(id: AnyHashable, bag: CancellationBag) -> Effect {
    return .fireAndForget {
      bag.cancel(id: id)
    }
  }
  
  public static func cancel(ids: [AnyHashable], bag: CancellationBag) -> Effect {
    return .fireAndForget {
      bag.cancel(ids: ids)
    }
  }
...
extension Reducer {
    public func optional(bag: @escaping (Environment) -> CancellationBag) -> Reducer<
        State?, Action, Environment
    > {
        .init { state, action, environment in
            guard state != nil else {
                bag(environment).cancelAll()
                return .none
            }
